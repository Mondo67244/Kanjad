...
class ParametresPage extends StatelessWidget {
  ...
  Future<void> _supprimerCompte(BuildContext context) async {
    ...
    try {
      final uid = user.id;
      // Delete data from 'Panier' and 'souhaits' tables
      await Supabase.instance.client
          .from('Panier')
          .delete()
          .eq('idUtilisateur', uid);
      await Supabase.instance.client
          .from('souhaits')
          .delete()
          .eq('idUtilisateur', uid);
      // Delete the user from the 'utilisateurs' table
      await Supabase.instance.client
          .from('utilisateurs')
          .delete()
          .eq('idUtilisateur', uid);
      // Delete the user from Supabase Auth
      await Supabase.instance.client.auth.signOut(); // Sign out first
      // await user.delete(); // La suppression d'un utilisateur depuis le client n'est pas autorisée par défaut.
      // Pour cela, vous devez créer une "Edge Function" sur Supabase.
      // Voici un exemple de fonction que vous pourriez créer dans /functions/delete-user/index.ts de votre projet Supabase :
      /*
      ...
      serve(async (req) => {
        try {
          // Créez un client Supabase avec le rôle de service pour avoir les droits d'admin
          const supabaseAdmin = createClient(
            Deno.env.get('SUPABASE_URL') ?? '',
            Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? ''
          )

          // Récupérez l'utilisateur à partir du token d'authentification de la requête
      ...
      // Et dans votre code Flutter, vous appelleriez cette fonction comme ceci :

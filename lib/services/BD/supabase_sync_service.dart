...
class SupabaseSyncService {
  ...
  Future<void> syncCategories(List<Map<String, dynamic>> localCategories) async {
    try {
    ...
    try {
      ...
      for (var localProduct in localProducts) {
        final existsInSupabase = supabaseProducts.any(
          (prod) => prod.idProduit == localProduct['id'].toString(),
        );
  ...
  Future<void> syncOrders(List<Map<String, dynamic>> localOrders, String userId) async {
    try {
      // Récupérer les commandes depuis Supabase
      final supabaseOrders = await _supabaseService.getCommandesParUtilisateur(userId);
      
      // Comparer et synchroniser
      for (var localOrder in localOrders) {
        final existsInSupabase = supabaseOrders.any(
          (order) => order.idCommande == localOrder['id'].toString(),
        );
  ...
  Future<void> syncInvoices(List<Map<String, dynamic>> localInvoices, String userId) async {
    try {
      // Récupérer les factures depuis Supabase
      final supabaseInvoices = await _supabaseService.getFacturesParUtilisateur(userId);
      
      // Comparer et synchroniser
      for (var localInvoice in localInvoices) {
        final existsInSupabase = supabaseInvoices.any(
          (invoice) => invoice.idFacture == localInvoice['id'].toString(),
        );
